...
export const AuthProvider = ({ children }) => {
    ...
    // Crea una sesión de Stripe Checkout y devuelve la URL a la que hay
    // que redirigir al usuario para pagar. La activación real de Premium
    // ocurre en el backend (Edge Function `stripe-webhook`) cuando Stripe
    // confirma el pago, no aquí.
    const iniciarCheckoutPremium = async () => {
        const { data, error } = await supabase.functions.invoke('stripe-create-checkout');
        if (error) {
            // Si la función respondió con un error controlado (4xx/5xx),
            // supabase-js lo expone en error.context; intentamos leer el
            // mensaje real que mandó nuestra Edge Function.
            let mensaje = error.message;
            try {
                const cuerpo = await error.context?.json();
                if (cuerpo?.error) mensaje = cuerpo.error;
            } catch { /* noop */ }
            throw new Error(mensaje || 'No se pudo iniciar el pago.');
        }
